...
public class UIManagerWrap
{
	...
	{
		L.BeginClass(typeof(UIManager), typeof(UnityEngine.Behaviour));
	...
	static int CloseUIWindow(IntPtr L)
	{
		try
		{
			...
			if (TypeChecker.CheckTypes(L, 1, typeof(UIWindowBase), typeof(bool), typeof(UICallBack)) && TypeChecker.CheckParamsType(L, typeof(object), 4, count - 3))
			{
			...
			else if (TypeChecker.CheckTypes(L, 1, typeof(string), typeof(bool), typeof(UICallBack)) && TypeChecker.CheckParamsType(L, typeof(object), 4, count - 3))
			{
	...
	static int ShowUI(IntPtr L)
	{
		try
		{
			...
			else if (count == 1 && TypeChecker.CheckTypes(L, 1, typeof(UIWindowBase)))
			{
				UIWindowBase arg0 = (UIWindowBase)ToLua.ToObject(L, 1);
				UIWindowBase o = UIManager.ShowUI(arg0);
				ToLua.Push(L, o);
