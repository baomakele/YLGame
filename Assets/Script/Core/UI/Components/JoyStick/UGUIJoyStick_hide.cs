...
public class UGUIJoyStick_hide : UIBase, IDragHandler, IEndDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    private Vector3 mRockerStartPos = Vector3.zero;
    void Start()
    {
        //计算摇杆块的半径
        mRadius =( bg.sizeDelta.x - content.sizeDelta.x) * 0.5f;
        referenceResolution = UIManager.UILayerManager.GetUICameraDataByKey("").m_root.GetComponent<CanvasScaler>().referenceResolution;
        ...
        mRockerStartPos = rockerRectTran.anchoredPosition3D;
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        UpdatePosInfo(eventData);
    }

    private void UpdatePosInfo(PointerEventData eventData)
    {
        ...
        float z = Mathf.Atan2(contentPostion.normalized.y, contentPostion.normalized.x) * 180 / Mathf.PI - 90;
        rockerRectTran.localEulerAngles = new Vector3(0, 0, z);

        Vector3 nor = (mRockerStartPos - content.anchoredPosition3D).normalized;
        rockerRectTran.anchoredPosition3D = mRockerStartPos - nor * bg.sizeDelta.x * 0.5f;
    }
