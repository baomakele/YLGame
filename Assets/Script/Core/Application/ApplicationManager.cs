...
public class ApplicationManager : MonoBehaviour 
{
    ...
    public bool m_useAssetsBundle = true;
    ...
    [Tooltip("是否在屏幕上显示log")]
    public bool m_printLogToWindow = false;
    ...
    /// 程序启动
    /// </summary>
    public IEnumerator AppLaunch()
    {
        DontDestroyOnLoad(gameObject);
        Application.runInBackground = true;
        Screen.sleepTimeout = SleepTimeout.NeverSleep;
        if (m_printLogToWindow)
        {
            gameObject.AddComponent<DebuggerLog>() ;
        }
    ...
    void SetResourceLoadType(bool useCache)
    {
        if (UseAssetsBundle)
        {
            ...
            bool state = HotUpdateManager.CheckLocalVersion();
            //HotUpdateManager.StartHotUpdate(HotUpdateManager.c_HotAdress, (info) =>
            //{
            //    Debug.LogError("pro ====" + info.m_loadState.progress);
            //});
