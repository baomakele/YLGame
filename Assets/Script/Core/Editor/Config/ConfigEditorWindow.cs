...
public class ConfigEditorWindow : EditorWindow
{
  // UILayerManager m_UILayerManager;
    ...
    void SelectConfigGUI()
    {
        ...
        if (mask.Length != 0)
        {
            if (m_currentSelectIndex >=0 && m_currentSelectIndex < mask.Length)
            {
        ...
        if (isConfigFold)
        {
            ...
            if (!m_configNameList.Contains(configName) && configName != "")
            {
                ...
                if (GUILayout.Button("新增", GUILayout.Width(position.width - 60)))
                {
                    ...
                    ConfigManager.ClearResourcesConfig();
                    ResourcesConfigManager.CreateResourcesConfig();
                    UnityEditor.AssetDatabase.Refresh();
                    //string loadConfigName = configName;
                    //configName = "";
                    //LoadConfig(loadConfigName);
                    //SelectConfigGUI();
                    // EditorCoroutineRunner.StartEditorCoroutine(LoadConfigIE(configName));
    ...
    IEnumerator LoadConfigIE(string configName)
    {
