...
public class ProtocolNetworkService : INetworkInterface
{
    ...
    public static Dictionary<string, List<Dictionary<string, object>>> ReadProtocolInfo(string content)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (currentStatus == AnalysisProtocolStatus.None)
            {
                ...
                {
                    string msgName = rgx.Match(currentLine).Groups[1].Value.ToLower();
    ...
    static void AddType(string currentLine, Dictionary<string, object> currentFeidInfo)
    {
        ...
        else if (currentLine.Contains("int64"))
        {
            currentFeidInfo.Add("type", TYPE_int64);
        }     
    ...
    public static void ReadMethodNameInfo(out Dictionary<int, string> methodNameInfo, out Dictionary<string, int> methodIndexInfo, string content)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            if (lines[i].Contains(","))
            {
                ...
                string indexName = res.Groups[2].Value.ToLower();
    ...
    int GetMethodIndex(string messageType)
    {
        try
        {
            string MessgID = messageType.ToLower() ;
            return m_methodIndexInfo[MessgID];
        }
        catch
        {
    ...
    NetWorkMessage Analysis(ByteArray bytes)
    {
        ...
        int Lenth = bytes.ReadUShort(); //消息长度
                                        //  bytes.ReadByte();  //模块名
        ...
        int re_len = bytes.Length - 4;
        try
        {
            msg.m_MsgCode = methodIndex;
            msg.m_LuaByte = bytes.CopyBytes(re_len);
            msg.m_MessageType = ProtoDic.GetProtoNameByProtoId(methodIndex); ;
        }
        catch
        {
        ...
        msg.m_data = AnalysisData(methodIndex ,bytes.ReadBytes(re_len));
    ...
    Dictionary<string, object> AnalysisData(int protoID, byte[] bytes)
    {
        //Debug.LogError("protoID:" + protoID + "       AnalysisData: " +  BitConverter.ToString(bytes));
        Google.Protobuf.MessageParser messageParser = ProtoDic.GetMessageParser(protoID);
        object toc = messageParser.ParseFrom(bytes);
        string messageTypeTemp = ProtoDic.GetProtoNameByProtoId(protoID);
        Dictionary<string, object> data = new Dictionary<string, object>();
        data.Add(messageTypeTemp, toc);
        //string fieldName = "";
        //string customType = "";
        //int fieldType = 0;
        //int repeatType = 0;
        ...
        //catch (Exception e)
        //{
        //    throw new Exception(@"AnalysisData Excepiton Data is ->" + messageTypeTemp
        //                + "<-\nFieldName:->" + fieldName
    ...
    List<byte> GetSendByte(string messageType, Dictionary<string, object> data)
    {
        try
        {
            string messageTypeTemp = messageType.ToLower();
            if (!m_protocolInfo.ContainsKey(messageTypeTemp))
            {
                throw new Exception("ProtocolInfo NOT Exist ->" + messageTypeTemp + "<-");
            }
