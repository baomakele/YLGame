...
public class NetworkManager
{
    ...
    public static void Init(string networkInterfaceName,string socketName)
    {
        ...
        Type socketType = Type.GetType(socketName);
    ...
    public static CallBackNetMsgLua responseForLua = null;
    ...
    public static void RegisterListenNetMsgForLua(CallBackNetMsgLua callFunc)
    {
        responseForLua = callFunc;
        s_network.m_messageLuaCallBack = responseForLua;

    }

    public static void RemoveNetMsgForLua()
    {
        if (responseForLua != null)
        {
            responseForLua = null;
        }
    }
    ...
    public static void SendMessage(IMessage obj)
    {
        ...
        buff.Bytes.AddRange(result);
        SendMessage(buff.Buffer);
    }
    ...
    public static void SendMessage(short messageID, byte[] msg)
    {
        if (IsConnect)
        {
            ByteArray buff = new ByteArray();
            UInt16 lengh = (UInt16)(msg.Length + 2);
            buff.WriteShort((UInt16)lengh);
            buff.WriteShort((UInt16)messageID);
            buff.Bytes.AddRange(msg);
            SendMessage(buff.Buffer);
        }
        else
        {
    ...
    static void ReceviceMeaasge(NetWorkMessage message)
    {
        ...
        {
            if (s_heatBeat != null && s_heatBeat.IsHeartBeatMessage(message))
            {
    ...
    static void Dispatch(NetWorkMessage msg)
    {
        try
        {
            ...
            if (s_network.m_messageLuaCallBack != null)
            {
                s_network. m_messageLuaCallBack((short)msg.m_MsgCode, msg.m_LuaByte);
            }
...
public delegate void CallBackNetMsgLua(short msgID, byte[] useByte);
