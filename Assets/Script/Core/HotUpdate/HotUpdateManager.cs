...
public class HotUpdateManager 
{
    ...
    public static string c_HotAdress = "https://yueli01.oss-cn-shanghai.aliyuncs.com/YLGame";
    ...
    static IEnumerator CheckVersion()
    {
        ...
        UnityWebRequest request = UnityWebRequest.Get(s_versionFileDownLoadPath);
        yield return request.SendWebRequest();
   
        while (!request.isDone)
        {
            UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingVersionFile, GetHotUpdateProgress(false, false, request.downloadProgress));
       ...
       // byte[] results = request.downloadHandler.data;
        ...
        while (request.error != null && request.error != "")
        {
            UpdateDateCallBack(HotUpdateStatusEnum.VersionFileDownLoadFail, 0);
            yield return null;
        }
        ...
        s_versionFileCache = ab.LoadAsset<TextAsset>(c_versionFileName).text;
        s_versionByteCache = results;
        ab.Unload(true);
    ...
    static void Init()
    {
        ...
        string downLoadPath = downLoadServicePath + "/" + platform  + "/";
